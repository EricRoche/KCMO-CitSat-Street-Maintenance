...
Zips.311 <- Data.311 %>%
  ...
  filter(ZIP.CODE != "na") %>%
  filter(grepl("Streetlight Out ",REQUEST.TYPE))
...
p+ geom_point()  +xlab("Number of Requests for Streetlight Out (any type)") +ylab("Mean Satisfaction with Street Maintenance in YOUR Neighborhood") + 
  ggtitle("CitSat By # of Streetlight Out Requests with a Loess Model")

p <- ggplot(Zip.Data, aes(Number.of.Requests, Mean.Satisfaction.Neighborhood.Streets)) + geom_smooth(method = "lm", level=0.90)
p+ geom_point()  +xlab("Number of Requests for Streetlight Out (any type)") +ylab("Mean Satisfaction with Street Maintenance in YOUR Neighborhood") + 
  ggtitle("CitSat By # of Streetlight Out Requests with a Linear Model")
