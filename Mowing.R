...
p+ geom_point()  +xlab("Number of Requests for Mowing (any type)") +ylab("Mean Satisfaction with Street Maintenance in YOUR Neighborhood") + 
  ggtitle("CitSat By # of Mowing 311 Requests with a Loess Model")

p <- ggplot(Zip.Data, aes(Number.of.Requests, Mean.Satisfaction.Neighborhood.Streets)) + geom_smooth(method = "lm", level=0.90)
p+ geom_point()  +xlab("Number of Requests for Mowing (any type)") +ylab("Mean Satisfaction with Street Maintenance in YOUR Neighborhood") + 
  ggtitle("CitSat By # of Mowing 311 Requests with a Linear Model")
