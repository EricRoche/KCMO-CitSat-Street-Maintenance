...
Zips.311 <- Data.311 %>%
  ...
  filter(ZIP.CODE != "na") %>%
  filter(grepl("Sign Missing|Street Sign Damage|Traffic Sign Down", REQUEST.TYPE))
...
p+ geom_point()  +xlab("Number of Requests for Signs") +ylab("Mean Satisfaction with Street Maintenance in YOUR Neighborhood")+
  ggtitle("CitSat By # of Sign Requests with a Loess Model")

p <- ggplot(Zip.Data, aes(Number.of.Requests, Mean.Satisfaction.Neighborhood.Streets)) + geom_smooth(method = "lm", level=0.90)  
p+ geom_point()  +xlab("Number of Requests for Signs") +ylab("Mean Satisfaction with Street Maintenance in YOUR Neighborhood")+
  ggtitle("CitSat By # of Sign 311 Requests with a Linear Model")
